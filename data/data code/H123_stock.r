######### create SF regressors for 1990-01 to 2100-12  ###########
rm(list=ls())
library(dplyr)
library(lubridate)
...
mt <- HolidayTime$mt   
J <- nrow(w)
Nmonth <- (year.end - year.start - 1) * 12 # number of months from 1990-01 to 2100-12
...
for(j in 2 : J) { # 1st row of w is (w1, w2, w3) = (0, 0, 0), i.e. no SF effects
  ...
  if(w1 != 0) {
    for(i in 1 : Nmonth) {
      if((mt[(i + 1)] >= before_start[i]) & (mt[(i + 1)] <= before_end[i])) { # last day of month i is within before-SF period of this year
        h1[i] <- (mt[(i + 1)] - before_start[i] + 1) / w1
      } else if ((mt[(i + 1)] >= before_start[(i + 1)]) & (mt[(i + 1)] <= before_end[(i + 1)])) { # last day of month i is within before-SF period of next year, i can only be Dec 
  ...
  if (w2 != 0) {
    for(i in 1 : Nmonth) {
      if ((mt[(i + 1)] >= during_start[i]) & (mt[(i + 1)] <= during_end[i])) { # last day of month i is within during-SF period of this year
        h2[i] <- 1 
      } else h2[i] <- 0
    }
  }
  ...
  if (w3 != 0) { 
    for(i in 1 : Nmonth) {
      if ((mt[(i + 1)] >= after_start[i]) & (mt[(i + 1)] <= after_end[i])) { # last day of month i is within after-SF period of this year
        h3[i] <-  (after_end[i] - mt[(i + 1)] + 1) / w3
      } else h3[i] <- 0
    }
  }
  ...
  outfile  <- paste("processed data/stock SF regressor/", j, ".txt", sep = "")
